...
export function initEstudiante() {
  ...
  if (inputDoc) inputDoc.maxLength = 10; // Límite para un documento estándar
...
async function cargarCursosEstudiante() {
  ...
  const res = await fetch(
    `http://localhost:3000/api/estudiante/${idEstudiante}/cursos`,
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
      },
  ...
  tbody.innerHTML = cursos
    .map((c) => `<tr><td>${c.nombre_materia}</td><td>${c.periodo}</td></tr>`)
    .join("");
}
...
async function cargarNotasEstudiante() {
  ...
  const res = await fetch(
    `http://localhost:3000/api/estudiante/${idEstudiante}/notas`,
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
      },
...
async function cargarHistorialEstudiante() {
  ...
  const res = await fetch(
    `http://localhost:3000/api/estudiante/${idEstudiante}/historial`,
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
      },
